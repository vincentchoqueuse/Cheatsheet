...
appliquer un gain :math:`K_1` de sorte à venir tangenter le contour isogain :math:`G_m` dB.
...
.. code ::
    ...
    K1dB = 14.2
    K1 = 10**(K1dB/20)
    nichols([F,K1*F],cm=[6,1,0,-1,-3,-6],show_phase=False,y_lim=[-25,30],x_lim=[-300,0])
...
.. code ::
    ...
    Ti = 1/(wr/5)
    Cp = tf([K1*Ti,K1],[Ti,0])
    nichols([F,K1*F,Cp*F],cm=[6,1,0,-1,-3,-6],show_phase=False,y_lim=[-25,30],x_lim=[-300,0])
...
.. code ::
    ...
    nichols([F,K1*F,Cp*F,Cp2*F],cm=[6,1,0,-1,-3,-6],show_phase=False,y_lim=[-25,30],x_lim=[-300,0])
